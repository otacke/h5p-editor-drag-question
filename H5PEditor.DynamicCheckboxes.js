...
H5PEditor.widgets.dynamicCheckboxes = H5PEditor.DynamicCheckboxes = (function ($) {
  ...
  C.prototype.setActive = function () {
    ...
    this.$item.html(html ? '<div class="h5peditor-label">' + this.field.label + '</div>' + (this.field.multiple ? '<a href="#" class="h5p-selectall">' + H5PEditor.t('H5PEditor.DragQuestionLERNLINIE', 'selectAll') + '</a>' : '') + '<ul class="h5peditor-dynamiccheckboxes-select">' + html + '</ul>' : '');
    ...
    updateSelectAll = function () {
      if ($checkboxes.length) {
        if ($checkboxes.length === $checkboxes.filter(':checked').length) {
          $a.addClass('h5p-deselectall').text(H5PEditor.t('H5PEditor.DragQuestionLERNLINIE', 'deselectAll'));
        }
        else {
          $a.removeClass('h5p-deselectall').text(H5PEditor.t('H5PEditor.DragQuestionLERNLINIE', 'selectAll'));
        }
      }
    };
    updateSelectAll();
...
// Get translations from H5PEditor.DragQuestionLERNLINIE
